...
function tagNameToFileName(tagName) {
    return `${tagName.toLowerCase().replaceAll(" ", "-")}.ts`;
}
function tagNameToVarName(tagName) {
    return tagName
        .toLowerCase()
        .replaceAll(/\s([a-zA-Z\d])/g, (captured) => captured.toUpperCase().trim());
}
export async function generateTags(basePath) {
    basePath = basePath || path.resolve();
    const tags = {};
    async function getTags(item) {
        async function scanDirectory(currentDirPath, omitPath) {
            ...
            for (const file of files) {
                ...
                frontmatter.tags?.forEach((tag) => {
                    ...
                    tags[tagName].push({
                        title: tagLabel ||
                            normalizePageTitle(frontmatter.sidebar_label || findPageTitle(fullPath) || ""),
                        path: `${BASE_URL}${item.tagBasePath}${frontmatter.slug ||
        ...
        for (const contentPath of item.contentPaths) {
            ...
            await scanDirectory(basePath, !contentPath.omitFromPath ? "" : contentPath.path);
        }
    }
    ...
    // write tags
    const files = [];
    await Promise.all(Object.keys(tags).map(async (tagName) => {
