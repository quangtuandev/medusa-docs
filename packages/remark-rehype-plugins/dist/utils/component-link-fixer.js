...
function matchMdLinks(str, linkOptions) {
    let linkMatches;
    // reset regex
...
function matchValueLink(str, linkOptions) {
    // reset index
...
export function componentLinkFixer(componentName, attributeName, options) {
    ...
    return async (tree, file) => {
        ...
        visit(tree, "mdxJsxFlowElement", (node) => {
            ...
            performActionOnLiteral(itemJsVar, (item) => {
                item.original.value = linkFn(item.original.value, linkOptions);
