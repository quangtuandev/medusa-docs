const cloudinaryImageRegex = /^https:\/\/res.cloudinary.com\/.*\/upload\/v[0-9]+\/(?<imageId>.*)$/;
export function cloudinaryImgRehypePlugin({ cloudinaryConfig, }) {
    return async (tree) => {
        if (!cloudinaryConfig) {
            return;
        }
        ...
        const { byRadius } = await import("@cloudinary/url-gen/actions/roundCorners");
        const { pad, imaggaScale, imaggaCrop, crop, fit, minimumPad, fill, scale, limitFit, thumbnail, limitFill, minimumFit, limitPad, fillPad, } = await import("@cloudinary/url-gen/actions/resize");
        const resizeActions = {
            pad,
            imaggaScale,
            imaggaCrop,
        ...
        visit(tree, (node) => {
            ...
            const image = cloudinary.image(matchingRegex.groups.imageId.replaceAll("%20", " "));
