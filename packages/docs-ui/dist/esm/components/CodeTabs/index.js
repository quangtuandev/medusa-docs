...
import { Badge, useColorMode, useTabs, } from "../..";
import clsx from "clsx";
import { CodeBlockActions } from "../CodeBlock/Actions";
import { CodeBlockHeaderWrapper } from "../CodeBlock/Header/Wrapper";
export const CodeTabs = ({ children, className, group = "client", blockStyle = "loud", }) => {
    ...
    const bgColor = useMemo(() => clsx(blockStyle === "loud" && "bg-medusa-contrast-bg-base", blockStyle === "subtle" && [
        colorMode === "light" && "bg-medusa-bg-component",
        colorMode === "dark" && "bg-medusa-code-bg-header",
    ]), [blockStyle, colorMode]);
    const boxShadow = useMemo(() => clsx(blockStyle === "loud" &&
    ...
    tabRefs.current = [];
    return (React.createElement("div", { className: clsx("my-docs_1 w-full max-w-full", "rounded-docs_lg", bgColor, boxShadow, className) },
        React.createElement(CodeBlockHeaderWrapper, { blockStyle: blockStyle, ref: codeTabsWrapperRef },
            React.createElement("span", { className: clsx("xs:absolute xs:transition-all xs:duration-200 xs:ease-ease xs:bottom-0", blockStyle === "loud" && "bg-medusa-contrast-fg-primary h-px", blockStyle === "subtle" && [
                    colorMode === "light" &&
            ...
            React.createElement("div", { className: "flex gap-docs_1 items-center" },
                selectedTab?.codeProps.badgeLabel && (React.createElement(Badge, { variant: selectedTab?.codeProps.badgeColor || "code", className: "!font-base" }, selectedTab.codeProps.badgeLabel)),
                React.createElement("ul", { className: clsx("!list-none flex gap-docs_0.75 items-center", "p-0 mb-0") }, Children.map(children, (child, index) => {
                    if (!React.isValidElement(child)) {
                        return React.createElement(React.Fragment, null);
                    }
                    return (React.createElement(child.type, { ...(typeof child.props === "object" ? child.props : {}), changeSelectedTab: changeSelectedTab, pushRef: (tabButton) => tabRefs.current.push(tabButton), blockStyle: blockStyle, isSelected: !selectedTab
                            ? index === 0
                            : selectedTab.value === child.props.value }));
                }))),
            actionsProps && React.createElement(CodeBlockActions, { ...actionsProps })),
