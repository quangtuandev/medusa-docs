...
export const CodeBlock = ({ source, hasTabs = false, lang = "", wrapperClassName, innerClassName, className, overrideColors = {}, collapsed = false, title = "", highlights = [], apiTesting = false, blockStyle = "loud", noCopy = false, noReport = false, noLineNumbers = false, children, collapsibleLines, expandButtonLabel, isTerminal, style, forceNoTitle = false, animateTokenHighlights, noAskAi = false, ...rest }) => {
    ...
    const apiRunnerRef = useRef(null);
    ...
    const hasInnerCodeBlock = useMemo(() => hasTabs || codeTitle.length > 0, [hasTabs, codeTitle]);
    const canShowApiTesting = useMemo(() => apiTesting !== undefined &&
        rest.testApiMethod !== undefined &&
        rest.testApiUrl !== undefined, [apiTesting, rest]);
    const bgColor = useMemo(() => clsx(overrideColors.bg, !overrideColors.bg && [
    ...
    const lineNumbersColor = useMemo(() => clsx(overrideColors.lineNumbersBg, !overrideColors.lineNumbersBg && [
    ...
    const borderColor = useMemo(() => clsx(overrideColors.border, !overrideColors.border && [
    ...
    const boxShadow = useMemo(() => clsx(overrideColors.boxShadow, !overrideColors.boxShadow && [
        blockStyle === "loud" &&
            "shadow-elevation-code-block dark:shadow-elevation-code-block-dark",
        blockStyle === "subtle" && "shadow-none",
    ]), [blockStyle, overrideColors]);
    const innerBgColor = useMemo(() => clsx(overrideColors.innerBg, !overrideColors.innerBg && [
        blockStyle === "loud" && [
    ...
    const innerBorderClasses = useMemo(() => clsx(overrideColors.innerBorder, !overrideColors.innerBorder && [
        blockStyle === "loud" && [
            hasInnerCodeBlock &&
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: clsx(hasInnerCodeBlock && "rounded-docs_lg", !hasInnerCodeBlock && "rounded-docs_DEFAULT", !hasTabs && boxShadow, blockStyle === "loud" && "code-block-highlight", blockStyle === "subtle" &&
                colorMode === "light" &&
                "code-block-highlight-light", wrapperClassName) },
            codeTitle && (React.createElement(CodeBlockHeader, { title: codeTitle, blockStyle: blockStyle, badgeLabel: rest.badgeLabel, badgeColor: rest.badgeColor, actionsProps: {
                    ...actionsProps,
                    inHeader: true,
                }, hideActions: hasTabs })),
            React.createElement("div", { className: clsx("relative mb-docs_1", "w-full max-w-full border code-block-elm", bgColor, borderColor, collapsed && "max-h-[400px] overflow-auto", hasInnerCodeBlock && "p-[5px] !pt-0 rounded-b-docs_lg", !hasInnerCodeBlock && "rounded-docs_DEFAULT", className), style: style },
                React.createElement(Highlight, { theme: codeTheme, code: source.trim(), language: language, ...rest }, ({ className: preClassName, style: { backgroundColor: _, ...style }, tokens, ...rest }) => (React.createElement("div", { className: clsx(innerBorderClasses, innerBgColor, "relative", innerClassName), ref: codeContainerRef },
                    collapsibleType === "start" && (React.createElement(React.Fragment, null,
                        React.createElement(CodeBlockCollapsibleButton, { type: collapsibleType, expandButtonLabel: expandButtonLabel, className: innerBorderClasses, ...collapsibleResult }),
                        React.createElement(CodeBlockCollapsibleFade, { type: collapsibleType, collapsed: collapsibleResult.collapsed, hasHeader: hasInnerCodeBlock }))),
                    React.createElement("pre", { style: { ...style, fontStretch: "100%" }, className: clsx("relative !my-0 break-words bg-transparent !outline-none", "overflow-auto break-words p-0 pr-docs_0.25", "rounded-docs_DEFAULT", !hasInnerCodeBlock &&
                            tokens.length <= 1 &&
                            "px-docs_1 py-[6px]", (noLineNumbers ||
                            (tokens.length <= 1 && !isTerminalCode)) &&
                            "pl-docs_1", preClassName), onCopy: trackCopy },
                        React.createElement("code", { className: clsx("text-code-body font-monospace table min-w-full print:whitespace-pre-wrap", "py-docs_0.75"), ref: codeRef },
                    ...
                    !hasInnerCodeBlock &&
                        (!noCopy || !noReport || canShowApiTesting || !noAskAi) && (React.createElement(CodeBlockActions, { ...actionsProps, inHeader: false, isSingleLine: tokens.length <= 1 })),
                    collapsibleType === "end" && isCollapsible(tokens) && (React.createElement(React.Fragment, null,
                        React.createElement(CodeBlockCollapsibleFade, { type: collapsibleType, collapsed: collapsibleResult.collapsed, hasHeader: hasInnerCodeBlock }),
                        React.createElement(CodeBlockCollapsibleButton, { type: collapsibleType, expandButtonLabel: expandButtonLabel, className: innerBorderClasses, ...collapsibleResult })))))))),
        canShowApiTesting && (React.createElement(CSSTransition, { unmountOnExit: true, in: showTesting, timeout: 150, classNames: {
            ...
            React.createElement(ApiRunner, { apiMethod: rest.testApiMethod, apiUrl: rest.testApiUrl, pathData: rest.testPathParams, bodyData: rest.testBodyParams, queryData: rest.testQueryParams, ref: apiRunnerRef })))));
};
