...
export const CodeBlockLine = ({ line, highlights = [], lineNumber, getLineProps, getTokenProps, showLineNumber, lineNumberColorClassName, lineNumberBgClassName, isTerminal, animateTokenHighlights = false, }) => {
    ...
    const getTokensElm = ({ tokens, isTokenHighlighted, offset, }) => (React.createElement("span", { className: clsx(isTokenHighlighted && "relative") },
        isTokenHighlighted && (React.createElement("span", { className: clsx(animateTokenHighlights && [
        ...
        tokens.map((token, key) => {
            ...
            return (React.createElement("span", { key: tokenKey, className: clsx(tokenClassName, isTokenHighlighted && "relative z-[1]"), ...rest }));
        })));
    const isHighlightedLine = useMemo(() => highlights.length !== 0 && highlightedTokens.length === 0, [highlights, highlightedTokens]);
    return (React.createElement("span", { key: lineNumber, ...lineProps, className: clsx("table-row", isHighlightedLine && "bg-medusa-alpha-white-alpha-6", lineProps.className) },
        (showLineNumber || isTerminal) && (React.createElement("span", { className: clsx("mr-docs_1 table-cell select-none", "sticky left-0 w-[1%] px-docs_1 text-right", lineNumberColorClassName, lineNumberBgClassName) }, isTerminal ? "❯" : showLineNumber ? lineNumber + 1 : "")),
        React.createElement("span", null, transformedLine.map(({ tokens, type, highlight }, index) => {
            ...
            return (React.createElement(React.Fragment, { key: index },
                tooltipText && (React.createElement(Tooltip, { text: tooltipText, tooltipClassName: "font-base", render: ({ content }) => (React.createElement(MarkdownContent, { allowedElements: ["a", "strong", "code", "br"], unwrapDisallowed: true }, content || "")) }, getTokensElm({
                    tokens,
