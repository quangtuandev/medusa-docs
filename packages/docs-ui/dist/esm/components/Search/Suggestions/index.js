...
export const SearchSuggestions = ({ suggestions }) => {
    ...
    return (React.createElement("div", { className: "h-full overflow-auto px-docs_0.5" },
        commands.length > 0 && (React.createElement(React.Fragment, null,
            React.createElement(SearchHitGroupName, { name: "Commands" }),
            commands.map((command, index) => (React.createElement(SearchSuggestionItem, { onClick: () => setCommand(command), key: index, tabIndex: index },
                command.icon,
                React.createElement("span", { className: "flex-1" }, command.title),
                command.badge && React.createElement(Badge, { ...command.badge })))))),
