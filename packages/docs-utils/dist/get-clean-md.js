...
import { parseCard, parseCardList, parseCodeTabs, parseColors, parseComponentExample, parseComponentReference, parseDetails, parseEventHeader, parseHookValues, parseIconSearch, parseNote, parsePackageInstall, parsePrerequisites, parseSourceCodeLink, parseSplitList, parseTable, parseTabs, parseTypeList, parseWorkflowDiagram, } from "./utils/parsers.js";
...
const isComponentAllowed = (nodeName) => {
    return Object.keys(parsers).includes(nodeName);
};
const parseComponentsPlugin = (options) => {
    return async (tree) => {
        const { visit } = await import("unist-util-visit");
        let pageTitle = "";
        visit(tree, ["mdxJsxFlowElement", "element", "mdxjsEsm", "heading"], (node, index, parent) => {
            ...
            if (parser) {
                ...
                return parser(node, index, parent, parserOptions);
            }
        });
    };
};
const removeFrontmatterPlugin = () => {
    return async (tree) => {
        const { visit } = await import("unist-util-visit");
        visit(tree, ["yaml", "toml"], (node, index, parent) => {
...
export const getCleanMd = async ({ file, plugins, parserOptions, type = "file", }) => {
