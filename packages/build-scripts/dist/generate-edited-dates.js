...
const monorepoRelativePath = projectBasePath
    .replace(rootPath, "")
    .replace(/^\//, "");
const generatedFilePath = path.join(projectBasePath, "generated", "edit-dates.mjs");
const getGitEditDatesOfPaths = async (paths) => {
    const editDates = {};
    await Promise.all(paths.map(async (filePath) => {
        const editDateProcess = await promiseExec(`git log -1 --pretty="format:%cI" ${filePath}`);
...
const getOsLastEditDates = async (paths) => {
    const editDates = {};
    await Promise.all(paths.map(async (filePath) => {
...
const getChangedFiles = async () => {
    const changedFiles = [];
    // get untracked files
    const untrackedFiles = await promiseExec(`git ls-files --other --exclude-standard`);
    ...
    // get tracked files
    const trackedFiles = await promiseExec(`{ git diff --name-only ; git diff --name-only --staged ; } | sort | uniq`);
    trackedFiles.stdout.split("\n").forEach((file) => {
        ...
        if (!file.startsWith(monorepoRelativePath) ||
...
export const generateEditedDates = async () => {
    ...
    if (generatedFileExists) {
        const existingEditDates = JSON.parse((await readFile(generatedFilePath, "utf-8")).replace(fileContentPrefix, ""));
    ...
    await writeFile(generatedFilePath, `${fileContentPrefix}${JSON.stringify(editDates, undefined, 2)}`, {
        flag: "w",
    });
};
