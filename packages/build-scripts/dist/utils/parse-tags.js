...
export const parseTags = (tagNames) => {
...
const getIntersectionTags = (tags) => {
    const tagsToIntersect = tags
        .split("+")
    ...
    return tagsToIntersect[0].filter((tagItem) => {
        return tagsToIntersect
            .slice(1)
            .every((otherTag) => otherTag.some((otherTagItem) => otherTagItem.path === tagItem.path));
    });
};
const getItemsToRemove = (tags) => {
