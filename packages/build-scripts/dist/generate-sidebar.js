...
async function getAutogeneratedSidebarItems(dir, nested = false) {
    const isRefDir = dir.startsWith("/references") && dir !== "/references-overview";
    ...
    for (const file of files) {
        ...
        if (fileBasename !== "page.mdx" && statSync(filePath).isDirectory()) {
            const newItems = await getAutogeneratedSidebarItems(filePath.replace(basePath, ""), true);
            if (nested && newItems.length > 1) {
                items.push({
                    type: "sub-category",
                    title: fileBasename.charAt(0).toUpperCase() + fileBasename.substring(1),
                    children: newItems,
                    loaded: true,
                });
            }
...
async function getAutogeneratedTagSidebarItems(tags, type, existingChildren) {
    const items = [];
    const parsedTags = parseTags(tags);
    items.push(...parsedTags
        .filter((tagItem) => {
...
function validateItem(item) {
    if (!item.type) {
        throw new Error(`ERROR: The following item doesn't have a type: ${JSON.stringify(item, undefined, 2)}`);
    }
    if (item.type === "sidebar" && !item.sidebar_id) {
        throw new Error(`ERROR: The following sidebar item doesn't have a sidebar_id: ${JSON.stringify(item, undefined, 2)}`);
    }
    if (item.type === "link" && !item.path) {
        throw new Error(`ERROR: The following link item doesn't have a path: ${JSON.stringify(item, undefined, 2)}`);
    }
}
async function checkItem(item) {
    ...
    if (item.autogenerate_path) {
        item.children = [
            ...(item.children || []),
            ...(await getAutogeneratedSidebarItems(item.autogenerate_path)).map((child) => {
        ...
        item.children = [
            ...(item.children || []),
            ...(await getAutogeneratedTagSidebarItems(item.autogenerate_tags, item.autogenerate_as_ref ? "ref" : "link", item.children)),
        ];
    }
    else if (item.custom_autogenerate &&
...
async function checkItems(items, options) {
    let updatedItems = (await Promise.all(items.map(async (item) => await checkItem(item)))).filter((item) => {
...
export async function generateSidebar(sidebars, options) {
    const path = await import("path");
    const { writeFileSync } = await import("fs");
    const normalizedSidebars = [];
    const { writeToFile = true, ...restOptions } = options || {};
    validateSidebarUniqueIds(sidebars);
    for (const sidebarItem of sidebars) {
        const hasCustomAutogenerator = sidebarItem.custom_autogenerate &&
    ...
    writeFileSync(path.resolve(generatedDirPath, "sidebar.mjs"), `export const generatedSidebars = ${JSON.stringify(normalizedSidebars, null, 2)}`, {
        flag: "w",
    });
}
